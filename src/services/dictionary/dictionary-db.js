...
import { DictionaryDBBase } from '../../core/language/dictionary-db-core.js';
import { DictionaryUtils } from '../../utils/dictionary-utils/dictionary-utils.js';
import { DictionaryEntry } from '../../core/data-structures/dictionary-entry.js';
...
export class DictionaryDB extends DictionaryDBBase {
    ...
    async initializeCacheManager() {
        try {
            const { DictionaryCacheManager } = await import('./dictionary-cache-manager.js');
     ...
     * エントリをデータベースに追加 (統一版)
     */
    async addEntry(entryOrWord, reading = null, definitions = [], synonyms = [], antonyms = [], pos = []) {
        let entry;
        
        // エントリオブジェクトまたは個別パラメータから作成
        if (typeof entryOrWord === 'object' && entryOrWord.word) {
            entry = entryOrWord;
        } else {
            entry = new DictionaryEntry(entryOrWord, reading, definitions, synonyms, antonyms, pos);
        }
    ...
    parseJMdictEntryOptimized(entryXml) {
        ...
        const xrefMatches = entryXml.match(/<xref>(.*?)<\/xref>/g); // Add this line for xref
        
        // 必須要素チェック
        const primaryWord = kanjiMatch ? kanjiMatch[1].trim() : (readingMatch ? readingMatch[1].trim() : null);
        if (!primaryWord || !DictionaryUtils.isValidJapaneseWord(primaryWord)) return null;
        ...
        if (posMatches) {
            for (let i = 0; i < Math.min(posMatches.length, 3); i++) {
                ...
                if (posMatch) {
                    posList.push(DictionaryUtils.normalizePOS(posMatch[1]));
                }
            }
        }
        ...
        if (xrefMatches) {
            for (const xrefMatch of xrefMatches) {
                const xrefWordMatch = xrefMatch.match(/<xref>(.*?)<\/xref>/);
                if (xrefWordMatch && xrefWordMatch[1]) {
                    const xrefWord = xrefWordMatch[1].trim();
                    // JMdictのxrefは通常、別のエントリへの参照なので、そのまま同義語として追加
                    extractedSynonyms.push(xrefWord);
                }
            }
        }
        ...
        const entry = new DictionaryEntry(
            ...
            definitions,
            extractedSynonyms, // Pass extracted synonyms here
            [], // 反義語は後で処理
            posList
        );
        ...
        while ((entryMatch = entryPattern.exec(xmlContent)) !== null) {
            ...
            try {
                ...
                for (const senseMatch of senseMatches) {
                    ...
                    posMatches.forEach(match => posList.add(DictionaryUtils.normalizePOS(match[1])));
                ...
                if (primaryWord && definitions.length > 0 && DictionaryUtils.isValidJapaneseWord(primaryWord)) {
                    const entry = new DictionaryEntry(
                        primaryWord,
                        primaryReading,
                    ...
                    // 頻度推定（簡易）
                    entry.frequency = DictionaryUtils.estimateFrequency(primaryWord, definitions);
                    entry.level = DictionaryUtils.estimateLevel(primaryWord, definitions);
    ...
    async buildEnhancedSynonymMap() {
        ...
        DictionaryUtils.calculateSynonymQualityScoresOptimized();
    ...
    buildSemanticGroupsOptimized(entries) {
        ...
        for (const entry of processEntries) {
            // 効率的キーワード抽出
            const keywords = DictionaryUtils.extractKeywordsOptimized(entry.definitions);
